...
function resolveImage(type: Type): string {
  return resolveAsset('ABTesting', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Firebase AB Testing');
      default:
        return SubLabel('AB Testing');
    }
  }
  return undefined;
}

export const ABTesting: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
