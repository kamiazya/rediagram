...
function resolveImage(type: Type): string {
  return resolveAsset('CloudMessaging', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Firebase Cloud Messaging');
      default:
        return SubLabel('Cloud Messaging');
    }
  }
  return undefined;
}

export const CloudMessaging: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
...
CloudMessaging.displayName = 'CloudMessaging';
