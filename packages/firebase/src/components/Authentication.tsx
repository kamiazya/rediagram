...
function resolveImage(type: Type): string {
  return resolveAsset('Authentication', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Firebase Authentication');
      default:
        return SubLabel('Authentication');
    }
  }
  return undefined;
}

export const Authentication: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
