...
function resolveImage(type: Type): string {
  return resolveAsset('CloudStorage', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Cloud Storage for Firebase');
      default:
        return SubLabel('Cloud Storage');
    }
  }
  return undefined;
}

export const CloudStorage: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
