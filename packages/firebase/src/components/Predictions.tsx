...
function resolveImage(type: Type): string {
  return resolveAsset('Predictions', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Firebase Predictions');
      default:
        return SubLabel('Predictions');
    }
  }
  return undefined;
}

export const Predictions: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
