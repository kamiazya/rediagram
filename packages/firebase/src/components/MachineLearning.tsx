...
function resolveImage(type: Type): string {
  return resolveAsset('MachineLearning', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Firebase Machine Learning');
      default:
...
export const MachineLearning: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
