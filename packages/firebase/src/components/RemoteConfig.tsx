...
function resolveImage(type: Type): string {
  return resolveAsset('RemoteConfig', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Firebase Remote Config');
      default:
        return SubLabel('Remote Config');
    }
  }
  return undefined;
}

export const RemoteConfig: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
...
RemoteConfig.displayName = 'RemoteConfig';
