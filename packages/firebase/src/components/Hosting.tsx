...
function resolveImage(type: Type): string {
  return resolveAsset('Hosting', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Firebase Hosting');
      default:
        return SubLabel('Hosting');
    }
  }
  return undefined;
}

export const Hosting: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
