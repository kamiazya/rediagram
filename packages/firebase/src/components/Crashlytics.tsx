...
function resolveImage(type: Type): string {
  return resolveAsset('Crashlytics', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Crashlytics');
      default:
        return SubLabel('Firebase Crashlytics');
    }
  }
  return undefined;
}

export const Crashlytics: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
