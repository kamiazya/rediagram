...
function resolveImage(type: Type): string {
  return resolveAsset('DynamicLinks', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Firebase Dynamic Links');
      default:
        return SubLabel('Dynamic Links');
    }
  }
  return undefined;
}

export const DynamicLinks: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
...
DynamicLinks.displayName = 'DynamicLinks';
