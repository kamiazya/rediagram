...
function resolveImage(type: Type): string {
  return resolveAsset('CloudFunctions', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Cloud Firestore for Firebase');
      default:
        return SubLabel('Cloud Functions');
    }
  }
  return undefined;
}

export const CloudFunctions: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
