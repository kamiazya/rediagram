...
function resolveImage(type: Type): string {
  return resolveAsset('CloudFirestore', `${type}.png`);
}
...
function useServiceName(): ReactElement | undefined {
  ...
  if (serviceName) {
    ...
    switch (type) {
      case 'full':
        return SubLabel('Cloud Firestore');
      default:
        return SubLabel('Firestore');
    }
  }
  return undefined;
}

export const CloudFirestore: FC<Props> = ({ name, type, children, ...dependences }) => {
  useAssertProvider();
