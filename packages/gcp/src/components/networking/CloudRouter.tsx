...
export const CloudRouter: FC<CloudRouterProps> = ({ name, description, children, ...dependences }) => {
  useAssertProvider();
  const icon = useIcon();
  const label = useLabelText(children, { defaultValue: name, htmlLike: true });
  return (
    <GCPNode service="Cloud Router" name={name} description={description} icon={icon} label={label} {...dependences} />
  );
};
