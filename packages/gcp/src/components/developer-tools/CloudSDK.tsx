...
export const CloudSDK: FC<CloudSDKProps> = ({ name, description, children, ...dependences }) => {
  useAssertProvider();
  const icon = useIcon();
  const label = useLabelText(children, { defaultValue: name, htmlLike: true });
  return (
    <GCPNode service="Cloud SDK" name={name} description={description} icon={icon} label={label} {...dependences} />
  );
};
